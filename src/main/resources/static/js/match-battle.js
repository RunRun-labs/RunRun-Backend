...
function checkAndRestoreState() {
  ...
  fetch("/api/battle/" + SESSION_ID + "/rankings", {
    ...
    .then((data) => {
      ...
      const countdownShown = sessionStorage.getItem(
        "battle_countdown_" + SESSION_ID
      );
...
let ttsMotivateTimerId = null; // ✅ 주기적인 응원(TTS) 타이머
  ...
  fetch("/api/match/session/" + SESSION_ID, {
    ...
    .then((data) => {
      ...
      if (sessionData.status !== "IN_PROGRESS") {
        console.warn(
          "⚠️ 세션이 아직 시작되지 않음: status=" + sessionData.status
        );
    ...
    .catch((error) => {
      ...
      alert(
        "세션 정보를 불러오지 못했습니다. (페이지는 유지됩니다 - 로그 확인용)"
      );
...
function onConnected(frame) {
  ...
  // ✅ 타임아웃 시작 메시지 구독
  stompClient.subscribe(
    "/sub/battle/" + SESSION_ID + "/timeout-start",
    function (message) {
      const data = JSON.parse(message.body);
  ...
  countdownInterval = setInterval(() => {
    ...
    if (count > 0) {
      ...
      setTimeout(async () => {
        ...
        // ✅ 시작 시점에 resetDistanceState 호출하여 이미 지나간 거리 TTS 스킵 (오프라인/솔로런과 동일)
        if (
          ttsReady &&
          window.TtsManager &&
          ...
          // 시작 시점이므로 남은 거리는 전체 목표 거리
          const remainingDistanceKm = sessionData.targetDistance;
          window.TtsManager.resetDistanceState(remainingDistanceKm);
        }

        startGPSTracking();
        startElapsedTimer();
        startMotivationSchedule(); // ✅ 추가: 응원 TTS 스케줄 시작
      }, 1000);
    }
  }, 1000);
}
...
function checkAllFinishedAndRedirect(rankings) {
  ...
  if (allFinished) {
    ...
    // ✅ 모든 참가자 완주 시 순위 TTS 재생 (아직 못 들은 사람들)
    showFinishMessage();
...
function createRankingItem(participant, isMe, allRankings) {
  ...
  if (isQuit) {
    ...
    pace.textContent = participant.isFinished
      ? "완주!"
      : `페이스 ${participant.currentPace} /km`;
  }
  ...
  if (isQuit) {
    ...
    positionIndicator.textContent = `-${gap.toFixed(0)}m (${
      participant.rank
    }위)`;
...
function renderPaceComparison(rankings) {
  ...
  others.forEach((participant, index) => {
    const card = createPaceCard({
      ...
      comparison: calculatePaceComparison(
        myData.currentPace,
        participant.currentPace
      ),
...
function handleFinish() {
  ...
  // ✅ 응원 TTS 타이머 중지
  if (ttsMotivateTimerId) {
    clearInterval(ttsMotivateTimerId);
    ttsMotivateTimerId = null;
  }
}
    ...
    if (remaining > 0) {
      ...
      if (remaining <= 10) {
        ...
        countdown.style.animation =
          "pulse 0.5s infinite, slideDown 0.5s ease-out";
      }
    } else {
      // ✅ 타임아웃 만료!
      ...
      `;
      countdown.style.background =
        "linear-gradient(135deg, rgba(153, 27, 27, 0.95), rgba(185, 28, 28, 0.95))";
      ...
      // ✅ 타임아웃 만료 시 순위 TTS 재생 (아직 못 들은 사람들)
      showFinishMessage();
