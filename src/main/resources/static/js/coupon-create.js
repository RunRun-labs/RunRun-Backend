document.addEventListener("DOMContentLoaded", function () {
  ...
  const validators = {
    ...
    code: (value, formData) => {
      const codeType = formData.codeType;

      // MULTI 타입일 경우 필수
      if (codeType === "MULTI") {
        if (!value || value.trim() === "") {
          return "MULTI 쿠폰은 쿠폰 코드(code)가 필수입니다.";
        }
      }
      ...
      if (value && value.trim() !== "") {
        if (value.length < 6 || value.length > 20) {
          return "쿠폰 코드(code)는 6~20자여야 합니다.";
        }
        if (!/^[A-Z0-9]+$/.test(value)) {
          return "쿠폰 코드(code)는 영문 대문자(A-Z)와 숫자(0-9)만 허용합니다.";
        }
        if (value.includes(" ")) {
          return "쿠폰 코드(code)는 공백을 포함할 수 없습니다.";
        }
      }
  ...
  if (createBtn && form) {
    createBtn.addEventListener("click", function (e) {
      ...
      // date 타입은 YYYY-MM-DD 형식이므로 T00:00:00을 추가
      const startDate = new Date(formData.startAt + "T00:00:00");
      ...
      // MULTI가 아니면 code를 null로
      if (requestData.codeType !== "MULTI") {
      ...
      fetch("/api/admin/coupons", {
        ...
        .then((data) => {
          if (data.success) {
            ...
            alert(
              "쿠폰 생성에 실패했습니다: " + (data.message || "알 수 없는 오류")
            );
          }
        })
        .catch((error) => {
          console.error("Error:", error);
          let errorMessage = "쿠폰 생성 중 오류가 발생했습니다.";
