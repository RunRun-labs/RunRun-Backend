...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadCouponCount() {
        // 결제 페이지에서 왔으면 API 대신 화면에 표시된 개수 사용
        const fromPayment = localStorage.getItem('paymentReturnUrl');

        if (fromPayment) {
            // 결제 페이지에서 온 경우: 쿠폰 목록 로드 후 개수 계산
    ...
    function loadCoupons(reset = false) {
      ...
      `;
        ...
        const fromPayment = localStorage.getItem('paymentReturnUrl');
        if (fromPayment) {
            // 멤버십 결제에 사용 가능한 쿠폰만 조회
            params.append("benefitTypes", "FIXED_DISCOUNT,RATE_DISCOUNT,EXPERIENCE");
        }
    ...
    function createCouponCard(coupon) {
        ...
        return `
      <div class="coupon-card" data-coupon-id="${couponIssueId}" ${fromPayment ? 'style="cursor: pointer;"' : ''} 
           ${fromPayment ? `onclick="selectCouponForPayment(${couponIssueId}, '${escapeHtml(coupon.name)}', '${coupon.benefitType}', ${benefitValue}, '${coupon.code || ''}')"` : ''}>
        <div class="coupon-card-inner">
          <div class="coupon-header">
            ...
            <div class="coupon-actions">
              ...
              <button 
                ...
                onclick="event.stopPropagation(); deleteCoupon(this);"
              >
                삭제
              </button>
            </div>
          ...
          ${fromPayment ? '<div class="select-indicator">선택하기 ›</div>' : ''}
        </div>
      </div>
    `;
    }
...
// 결제용 쿠폰 선택 (전역 함수)
window.selectCouponForPayment = function (couponIssueId, name, benefitType, benefitValue, code) {

    if (!code) {
        alert('쿠폰 코드 정보가 없습니다.');
        return;
    }
    ...
    if (benefitType !== 'FIXED_DISCOUNT'
        ...
        alert('이 쿠폰은 멤버십 결제에 사용할 수 없습니다.');
        return;
    }
    ...
    window.location.href = returnUrl || '/payment/pay';
};
