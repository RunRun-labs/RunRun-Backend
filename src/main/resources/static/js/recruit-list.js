document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayRecruits(recruits) {
    ...
    recruitList.innerHTML = recruits.map((recruit) => {
      const meetingDate = formatDateTime(recruit.meetingAt);
      const ageRange = recruit.ageMin && recruit.ageMax
          ? `${recruit.ageMin}~${recruit.ageMax}세` : "";
      const genderLimit = getGenderLimitText(recruit.genderLimit);
      const distanceText = recruit.distanceKm
          ? `현재 위치에서 ${recruit.distanceKm}km` : "";

      return `
          <div class="recruit-card" data-recruit-id="${recruit.recruitId}">
            <h3 class="card-title">제목 : ${recruit.title}</h3>
            <p class="card-info">뛸 거리 : ${recruit.targetDistance || 0}km</p>
            ${distanceText ? `<p class="card-distance">${distanceText}</p>`
          : ""}
            <div class="card-meeting">
              <svg class="card-meeting-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="card-tags">
              <div class="card-tag">
                <svg class="card-tag-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span>${recruit.currentParticipants
      || 1}/${recruit.maxParticipants}</span>
              </div>
              ${ageRange ? `<span>${ageRange}</span>` : ""}
              ${genderLimit ? `<span>${genderLimit}</span>` : ""}
              ${recruit.targetPace ? `<span>${recruit.targetPace}</span>` : ""}
            </div>
          </div>
        `;
    }).join("");
