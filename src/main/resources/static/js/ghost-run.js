...
let ghostStatusTtsSpoken = false; // 상태 변경 시 TTS 재생 여부 추적
...
async function handleStart() {
  ...
  // ✅ 주기적인 응원 TTS 스케줄 시작 (3분 후 1회, 이후 5분마다)
  if (!ttsMotivateTimerId && ttsReady && window.TtsManager) {
    setTimeout(() => {
      if (ttsReady && !completedHandled && !isFinished && isRunning) {
        window.TtsManager.speak("MOTIVATE_GOOD_JOB");
      }
    }, 3 * 60 * 1000);
    ttsMotivateTimerId = setInterval(() => {
      if (!ttsReady || completedHandled || isFinished || !isRunning) return;
      window.TtsManager.speak("MOTIVATE_GOOD_JOB");
    }, 5 * 60 * 1000);
  }
  ...
  if (distance >= 3) {
    ...
    try {
      ...
      if (speedMps != null && Number.isFinite(speedMps)) {
        const canAlert =
          currentTime - lastTooFastAlertAt > tooFastAlertCooldownMs;
...
function sendGPSData() {
  ...
  console.log(`📤 GPS 전송 -> ${destination}`, {
    ...
    targetDistance: ghostData?.targetDistance
...
function updateDistanceUI() {
  ...
  if (
    ...
    const remainingDistanceKm = Math.max(
      0,
      ghostData.targetDistance - totalDistanceKm
    );
