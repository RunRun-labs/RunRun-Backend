...
function validateProductName() {
    ...
    if (!v)
        return setFieldError(el, 'errProductName', '필수 입력 항목입니다.');
    if (v.length < 2)
...
function validateProductDescription() {
    ...
    const v = el.value.trim();
    if (!v)
        return setFieldError(el, 'errProductDescription', '필수 입력 항목입니다.');
    if (v.length < 10)
        return setFieldError(
            el,
            'errProductDescription',
            '상품 설명은 10자 이상이어야 합니다.'
        );
    if (v.length > 1000)
        return setFieldError(
            el,
            'errProductDescription',
            '상품 설명은 1000자 이하여야 합니다.'
        );
...
function validateRequiredPoint() {
    ...
    const raw = el.value;
    if (raw === '' || raw === null)
        return setFieldError(el, 'errRequiredPoint', '필수 입력 항목입니다.');

    const v = Number(raw);
    if (!Number.isFinite(v))
        return setFieldError(el, 'errRequiredPoint', '숫자만 입력 가능합니다.');
    if (v < 1)
        return setFieldError(
            el,
            'errRequiredPoint',
...
function clearValidationErrors() {
    ...
    setFieldError(
        document.getElementById('requiredPoint'),
        'errRequiredPoint',
        null
    );
    setImageError(null);
...
function initializeEventListeners() {
    ...
    document
        ...
        .addEventListener('blur', validateProductName);
    document
        .getElementById('productDescription')
        .addEventListener('blur', validateProductDescription);
    document
        .getElementById('requiredPoint')
        .addEventListener('blur', validateRequiredPoint);

    document
        .getElementById('productDescription')
        .addEventListener('focus', function () {
            validateUpTo('productDescription');
        });

    document.getElementById('requiredPoint').addEventListener('focus', function () {
        validateUpTo('requiredPoint');
    });
...
function renderProductTable(products) {
    ...
    tbody.innerHTML = products
        ...
        <tr>
            ...
            <td data-label="#">${currentPage * 10 + index + 1}</td>
            <td data-label="이미지">
                <img src="${imageUrl}" 
                     alt="${product.productName}" class="product-image">
            </td>
            <td data-label="상품명">${product.productName}</td>
            <td data-label="필요 포인트">${product.requiredPoint.toLocaleString()} P</td>
            <td data-label="판매 상태">
                <span class="status-badge ${
                product.isAvailable ? 'status-active' : 'status-inactive'
            }">
                    ${product.isAvailable ? '판매중' : '판매중지'}
                </span>
            </td>
            <td data-label="생성일">${formatDate(product.createdAt)}</td>
            <td data-label="수정일">${formatDate(product.updatedAt)}</td>
            <td data-label="관리">
...
async function handleFormSubmit(e) {
    ...
    const productDescription = document
        .getElementById('productDescription')
        .value.trim();
    const requiredPoint = parseInt(
        document.getElementById('requiredPoint').value,
        10
    );
...
async function deleteSelectedProducts() {
    ...
    if (
        !confirm(`선택한 ${selectedProducts.size}개의 상품을 삭제하시겠습니까?`)
    ) {
        return;
    }
