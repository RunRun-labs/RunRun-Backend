 ...
 *
 * @author : kimyongwon
 * @description : 친구 관련 비즈니스 로직 처리 서비스 현재 로직은 REJECTED 상태 재요청 불가
...
public class FriendService {
  ...
  public Long sendRequest(CustomUser principal, Long targetUserId) {
    ...
    try {
      ...
      log.info("친구 요청 알림 발송 완료 - friendId: {}, requesterId: {}, receiverId: {}",
          savedFriend.getId(), requester.getId(), receiver.getId());
    } catch (Exception e) {
      log.error("친구 요청 알림 생성 실패 - friendId: {}, requesterId: {}, receiverId: {}",
          savedFriend.getId(), requester.getId(), receiver.getId(), e);
    }
