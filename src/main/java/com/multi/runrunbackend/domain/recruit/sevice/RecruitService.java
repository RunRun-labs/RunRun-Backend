  ...
  public void leaveRecruit(Long recruitId, CustomUser principal) {
    ...
    if (recruit.getUser().getId().equals(user.getId())) {
      ...
      if (nextLeader.isPresent()) {
        User newHost = nextLeader.get().getUser();
        recruit.changeHost(newHost);

        try {
          notificationService.create(
              newHost,
              "모집글 방장 위임",
              "방장이 나가셔서 모집글 방장으로 위임되었습니다.",
              NotificationType.MATCH,
              RelatedType.RECRUIT,
              recruitId
          );
          log.info("모집글 방장 위임 알림 발송 완료 - recruitId: {}, newHostId: {}",
              recruitId, newHost.getId());
        } catch (Exception e) {
          log.error("모집글 방장 위임 알림 발송 실패 - recruitId: {}, newHostId: {}",
